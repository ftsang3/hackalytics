...
@author: Frank Tsang and Lokranjan Lakshmikanthan
"""
...
# assumes that for each plate appearance, there is only 1 pitcher and 1 batter (they do not change) --> used to
# determine pitcher name, pitcher side, batter name, etc. speed = RelSpeed


# change csv inputs here

data_file_name = "20220412-GeorgiaTech-1_unverified.csv"  # change the text inside quotations to the file name

folder_name = data_file_name[1:-4] + " Images"  # sets name of folder

if not os.path.isdir(folder_name):  # creates folder if it doesn't exist
    os.mkdir(folder_name)

df = pd.read_csv(data_file_name)  # reads data
...
pitchdict = OrderedDict()  # data structure that we use to parse through

# populate pitch dict with info
for pitchnumber in df.index:
    if (df["Inning"][pitchnumber], df["Top/Bottom"][pitchnumber], df["PAofInning"][pitchnumber]) in pitchdict:
        pitchdict[df["Inning"][pitchnumber], df["Top/Bottom"][pitchnumber], df["PAofInning"][pitchnumber]].append(
            pitchnumber)
    else:
        pitchdict[df["Inning"][pitchnumber], df["Top/Bottom"][pitchnumber], df["PAofInning"][pitchnumber]] = [
            pitchnumber]

pitchdictkeys = list(pitchdict.keys())  # list of all plate appearances in csv

for uniquepa in pitchdictkeys:  # plots each plate appearance
    pitchnumberslist = pitchdict[uniquepa]  # list of pitches in the plate appearance
    ...
    fig = plt.figure(figsize=(10, 10))  # plots plate appearance
    ax = plt.axes(projection='3d')
    ax.grid()

    # if statements set the view
    if batterside == "Left":
    ...
    # finds all the critical values needed for plotting for the pitch
    for pitchno in pitchnumberslist:
        ...
        ax.scatter(xfunc(zonetime), yfunc(zonetime), zfunc(zonetime), color="yellow", s=labelsize,
                   marker="$" + str(pitchofpa) + "$")
    ...
    # adding distinguishing features of baseball field to plot
    ...
    # names plot and saves as image in the folder
    ...
    # reformats image with proper crop
    if (os.path.exists(folder_plotname)):
        im = Image.open(folder_plotname)
        ...
        cropped.save(folder_plotname, format="png")

    # self.canvas.draw()

#code follows same format but this for loop is for plotting the batting
for uniquepa in pitchdictkeys:
    ...
    for pitchno in pitchnumberslist:
        ...
        if np.isnan(pitchtraj_x2):
            ...
            # set distinguishing features of baseball field
            leftfieldline = FancyArrow(.45, 0, 220, 232.63, width=0.3, length_includes_head=False, head_width=None,
                                       head_length=None, shape='full', color='b')
            ...
            if (os.path.exists(folder_plotname)):
                ...
                left = width / 9
                top = height / 5
                right = width - (width / 17)
                bottom = height - (height / 2.5)
                cropped = im.crop((left, top, right, bottom))
                cropped.save(folder_plotname, format="png")
        ...
        # self.canvas.draw()
            ...
            ax.scatter(x0 + x1 * hangtime + x2 * hangtime ** 2, -y0 - y1 * hangtime - y2 * hangtime ** 2,
                       z0 + z1 * hangtime + z2 * hangtime ** 2)
            ...
            #adds the distinguishing objects to the graph that were created earlier
            ...
            '''
            x = [
                x0 + x1 * time + x2 * time ** 2 + x3 * time ** 3 + x4 * time ** 4 + x5 * time ** 5 + x6 * time ** 6 + x7 * time ** 7 + x8 * time ** 8
                for time in t]
            y = [
                -y0 - y1 * time - y2 * time ** 2 - y3 * time ** 3 - y4 * time ** 4 - y5 * time ** 5 - y6 * time ** 6 - y7 * time ** 7 - y8 * time ** 8
                for time in t]
            z = [
                z0 + z1 * time + z2 * time ** 2 + z3 * time ** 3 + z4 * time ** 4 + z5 * time ** 5 + z6 * time ** 6 + z7 * time ** 7 + z8 * time ** 8
                for time in t]
            ax.plot3D(x, y, z)

            ax.scatter(
                x0 + x1 * hangtime + x2 * hangtime ** 2 + x3 * hangtime ** 3 + x4 * hangtime ** 4 + x5 * hangtime ** 5 + x6 * hangtime ** 6 + x7 * hangtime ** 7 + x8 * hangtime ** 8,
                -y0 - y1 * hangtime - y2 * hangtime ** 2 - y3 * hangtime ** 3 - y4 * hangtime ** 4 - y5 * hangtime ** 5 - y6 * hangtime ** 6 - y7 * hangtime ** 7 - y8 * hangtime ** 8,
                z0 + z1 * hangtime + z2 * hangtime ** 2 + z3 * hangtime ** 3 + z4 * hangtime ** 4 + z5 * hangtime ** 5 + z6 * hangtime ** 6 + z7 * hangtime ** 7 + z8 * hangtime ** 8)
...
# we now have a folder with all batting and pitching plot for each plate appearance
# We can start generating the report
# For loop parses through each unique plate appearance and adds the images and text where necessary
for uniquepa in pitchdictkeys:
    pitchnumberslist = pitchdict[uniquepa]
    ...
    for pitchno in pitchnumberslist:
        ...
        a = {"PitchCall": str(pitchcall), "B-S": (str(balls) + "-" + str(strikes)), "Speed": (str(speed)[:4] + " mph "),
             "PitchType": str(pitchtype)}
        data1.append(a)
        i += 1

        pitchframe = pd.DataFrame(data1, columns=[*data1[0]]) #converts list of dicts to dataframe for the plate appearance

        if pitchno == pitchnumberslist[-1]: #if the pitch num is the last pitch in the list
            if playresult != "Undefined":
                listpitchbat.append(
                    "P: " + str(pitchername) + " (" + pitcherside + ") - " + str(numouts) + " Out(s) \n" + "B: " + str(
                        battername) + " (" + batterside + ")" + " - " + str(playresult))
            elif korbb != "Undefined":
                listpitchbat.append(
                    "P: " + str(pitchername) + " (" + pitcherside + ") - " + str(numouts) + " Out(s) \n" + "B: " + str(
                        battername) + " (" + batterside + ")" + " - " + str(korbb))
            else:
                listpitchbat.append(
                    "P: " + str(pitchername) + " (" + pitcherside + ") - " + str(numouts) + " Out(s) \n" + "B: " + str(
                        battername) + " (" + batterside + ")" + " - " + str(pitchcall))

    pitchframe.index += 1
    listpitches.append(pitchframe)
    data1 = []

innum = pitchdictkeys[len(pitchdictkeys) - 1][0] #number of innings
numpages = math.ceil(len(pitchdictkeys) / 2)  #number of pages in pdf report
stopindexes = [i for i in range(0, (numpages * 2) - 1, 2)] #indexes at which to start a new page

scorelist = [] #used to store score
innings = [i for i in range(1, innum + 1)]  #list of inning numbers
teams = ['Top', 'Bottom']

#creates scoreboard and saves as a dataframe in scorelist
...
intcounter = 0
#adds plate appearance summary table for each plate appearance
...
# when done with for loop which parses through all plate appearances, export the pdf into the goal directory
